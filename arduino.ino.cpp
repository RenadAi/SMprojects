...
void loop() {
    ...
    if (command == "right" || command == "—«Ì " || command == "Right") {
        base.write(base_angle -= 20);
    }
    if (command == "left" || command == "Left" || command == "·› ") {
        base.write(base_angle += 20);
    }

    if (command == "top" || command == " Ê»" || command == "Top") {
        shoulder.write(shoulder_angle -= 20);
    }

    if (command == "bottom" || command == "»Ê Ê„" || command == "Bottom") {
        shoulder.write(shoulder_angle += 20);
    }
